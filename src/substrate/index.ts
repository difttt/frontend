...
async function getTriggers() {
  const exposures = await api.query.diftttModule.trigerOwner.entries(
    Alice.address,
  )
...
async function getActions() {
  const exposures = await api.query.diftttModule.actionOwner.entries(
    Alice.address,
  )
...
async function getRecipes() {
  const exposures = await api.query.diftttModule.recipeOwner.entries(
    Alice.address,
  )
